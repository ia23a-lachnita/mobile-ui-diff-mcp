...
export class OpenRouterProvider implements IModelJudgeProvider {
  ...
  async analyze(bundle: EvidenceBundle, allEvidence: Evidence[]): Promise<Evidence[]> {
    ...
    const prompt = [
      ...
      'You MUST emit at least one evidence item. An empty evidence array is NEVER acceptable.',
      'If the UI looks correct and matches the mockup, emit a polarity:"match" item summarizing what you see.',
      'Example match item: {"claimId":"overall-match","subject":"roi:' + bundle.roiId + '","polarity":"match","claim":"Visual layout matches the expected mockup.","confidence":0.9,"severity":"info","blocking":false}',
      '',
      'CRITICAL RULES:',
      ...
      '',
      'Return a JSON object with key "evidence": array of items. Each item must have EXACTLY these fields:',
      '  claimId (string), subject (string, use "roi:' + bundle.roiId + '"),',
      ...
      'Optional fields (include only if relevant): source (string), proposedChangeVector (string), expectedValue, actualValue',
      'Do NOT include any other fields (e.g. no referenceFactIds, notes, reason, or custom keys).',
      '',
      'polarity rules:',
